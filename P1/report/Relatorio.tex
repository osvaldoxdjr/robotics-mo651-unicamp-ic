...
A equação \ref{eq:mudvar} é utilizada para rotacionar e transladar o ponto de um referencial local para o global, respeitando princípio de não comutatividade.
Para extrair características do mapeamento de uma cena é possível utilizar a transformada de Hough que é uma técnica matemática que realiza a detecção de formas geométricas em imagens digitais, no caso específico de um mapa 2D seria a detecção de linhas.\\
...
\begin{enumerate}
	...
	\item Localização ground truth
	\item Localização odometria
